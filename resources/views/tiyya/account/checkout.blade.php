...
<body>
                ...
                <div style="max-width: 800px; margin: 40px auto; padding: 0 20px;">
                    ...
                    <div style="margin-bottom: 40px;">
                        ...
                        <div class="radio-card"
                            ...
                            <br />
                            <span><small><i>({{ $localisation->message }} )</i></small></span>
            ...
            <div class="dropdown-content" id="dropdown-content">
                <div class="cart-centered">
                    ...
                    <div class="cart-summary">
                        <span style="padding-top: 50px;">@lang('checkout.subtotal') . articles <span id="item-count">0</span> <span id="subtotal">0,00
                        ...
                        <span style="padding-top: 50px;" class="total">@lang('checkout.total'): <span id="total">0,00
                                {{ $settings->currency }}</span></span>
                        <small style="padding-top: 50px;" style="text-align: start !important;">@lang('checkout.taxes_including') <small id="taxes">
                                </small> incluse</small>
                    </div>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            cart.forEach(item => {
                ...
                li.textContent =
                    `${item.product_title} - ${item.price * item.quantity} {{ $settings->currency }}`;
            ...
            document.getElementById('total').textContent = `${subtotal.toFixed(2)} {{ $settings->currency }}`;
        });
    </script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (cart) {
                cart = JSON.parse(cart); // Parse the cart data
                const cartItemsList = document.getElementById('cart-items-list');
                ...
                cart.forEach(item => {
                    ...
                    `;
                    const productString =
                        `{${item.id},${item.name},${item.price},${item.quantity},${item.unit}}`; // Format as string
                    productsArray.push(productString)
                    cartItemsList.appendChild(li); // Append the list item to the cart-items-list
                    subtotal += item.price * item.quantity; // Correctly accumulate the subtotal
                });
                ...
                if (subtotal < {{ $settings->free_delivery_threshold }}) {
                    subtotal = subtotal + {{ $settings->normal_delivery_price }};
